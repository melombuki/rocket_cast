class Api::PodcastsController < ApiController
  ...
  def update
    updatePodcastFromUrl(params[:url])
  end
  ...
  def updatePodcast(podcast)
    ...
    syndFeed = Feedjira::Feed.fetch_and_parse podcast.url 
    ...
    podcastEntryIds = podcast.entries.map{|e| e.entry_id}

    # byebug

    # Add all entries to the podcast
    syndFeed.entries.select {|e| !podcastEntryIds.member?(e.entry_id)}.each { |syndEntry|
      puts "This will be newly added #{syndEntry.title}"
      entry = Entry.new
      ...
      entry.entry_id = syndEntry.entry_id
